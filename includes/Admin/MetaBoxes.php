...
class MetaBoxes {
	...
	public function register(): void {
		add_meta_box(
			...
			__( '3D Model Settings', '3d-model-viewer' ),
			array( $this, 'render' ),
	...
	public function render( \WP_Post $post ): void {
		...
		<div class="wp3ds-admin-fields">
			<div class="notice notice-info inline">
				<p><strong><?php esc_html_e( 'How to embed this 3D item', '3d-model-viewer' ); ?></strong></p>
				<p><?php esc_html_e( 'Paste one of these shortcodes into any post, page, or widget that supports shortcodes.', '3d-model-viewer' ); ?></p>
				<p><code><?php echo esc_html( $shortcode_by_id ); ?></code></p>
				<?php if ( '' !== $shortcode_by_slug ) : ?>
					<p><code><?php echo esc_html( $shortcode_by_slug ); ?></code></p>
				<?php else : ?>
					<p class="description"><?php esc_html_e( 'Save or publish this item to generate a slug-based shortcode too.', '3d-model-viewer' ); ?></p>
				<?php endif; ?>
			</div>
			<p>
				<label for="wp3ds_model_url"><strong><?php esc_html_e( 'GLB File', '3d-model-viewer' ); ?></strong></label><br>
			...
			<p>
				<button type="button" class="button" data-media-target="#wp3ds_model_url" data-media-id-target="#wp3ds_model_attachment_id" data-allowed-extension="glb" data-media-title="<?php echo esc_attr__( 'Select GLB File', '3d-model-viewer' ); ?>" data-media-button="<?php echo esc_attr__( 'Use this GLB file', '3d-model-viewer' ); ?>"><?php esc_html_e( 'Select GLB', '3d-model-viewer' ); ?></button>
				<button type="button" class="button-link-delete" data-clear-media="#wp3ds_model_url" data-clear-media-id="#wp3ds_model_attachment_id"><?php esc_html_e( 'Remove file', '3d-model-viewer' ); ?></button>
			</p>
			<p class="description"><?php esc_html_e( 'Only self-hosted .glb files from the WordPress Media Library are accepted.', '3d-model-viewer' ); ?></p>

			<p>
				<label for="wp3ds_bg_color"><strong><?php esc_html_e( 'Background Color', '3d-model-viewer' ); ?></strong></label><br>
				<input type="color" id="wp3ds_bg_color" name="wp3ds_bg_color" value="<?php echo esc_attr( $bg_color ); ?>">
			</p>

			<p>
				<label for="wp3ds_explode_step"><strong><?php esc_html_e( 'Explode Strength', '3d-model-viewer' ); ?></strong></label><br>
			...
			<p>
				<label>
					...
					<?php esc_html_e( 'Enable auto-rotate by default', '3d-model-viewer' ); ?>
				</label>
			</p>
			...
			<div class="wp3ds-explode-parts" data-explode-parts="<?php echo esc_attr( $explode_parts ?: '[]' ); ?>">
				...
				<div class="wp3ds-explode-parts__header">
					<strong><?php esc_html_e( 'Explode View Parts', '3d-model-viewer' ); ?></strong>
					<p class="description"><?php esc_html_e( 'Mesh parts are detected automatically from the GLB model. Fine-tune the explode direction and the part details shown in the front-end info card.', '3d-model-viewer' ); ?></p>
				</div>
				<div class="wp3ds-explode-parts__status" data-parts-status><?php esc_html_e( 'Select a GLB file to detect model parts.', '3d-model-viewer' ); ?></div>
